...
class ReactiveMongoConnector(config: play.api.Configuration, lifecycle: ApplicationLifecycle) extends ReactiveMongoComponent {
  ...
  val _mongoConnector: Option[MongoConnector] = {
    ...
    val mongoConfig = config.getConfig("mongodb")
      //.getOrElse(config.getConfig(s"${app.Dev}.mongodb")
